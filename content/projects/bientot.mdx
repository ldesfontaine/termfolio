...
Bientôt est un système de monitoring agent/serveur en Go. Un agent léger par machine, un serveur central. L'agent auto-détecte ce qui est installé (Docker, ZFS, CrowdSec, Traefik, NetBird…), collecte les métriques et les push vers le serveur. Le dashboard s'adapte dynamiquement aux modules détectés.
